...
也可以在 GitHub Actions 页面手动运行，并输入 `1.0.5` 或 `v1.0.5`。下载与目标机器 CPU 架构匹配的镜像包后，可在离线机器执行：

```bash
# x86-64 机器
sha256sum -c ddt-insight-1.0.5-linux-amd64.tar.gz.sha256
gzip -dc ddt-insight-1.0.5-linux-amd64.tar.gz | docker load

# ARM64 / AArch64 机器
sha256sum -c ddt-insight-1.0.5-linux-arm64.tar.gz.sha256
gzip -dc ddt-insight-1.0.5-linux-arm64.tar.gz | docker load
```
