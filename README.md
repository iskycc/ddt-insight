...
- 导入以持久化后台任务执行，支持进度查看、取消、异常重启续跑和来源追踪，并永久记录未导入表格及具体原因
- 从 `data` Sheet 读取普通用例，或将大小写不敏感的 `step1` 至 `stepN` 对应行组合为用户旅程
...
| 导入来源 | 工作台“导入来源” | 有效表格继续导入，问题表格被排除；按状态、策略、人员、任务号和文件名分页追踪，并保留每个失败表格的具体原因 |
...
| 未登录统计大盘 | `/` | 不登录可以查看平台统计 |
| 登录后用例管理 | `/workspace` | 所有登录用户可查看；编辑员与管理员可维护用例，查看者写接口返回 403，接口无 Session 返回 401 |
| 用户与角色 | 用户管理页面 | 三种角色均可查看全部菜单；查看者全局只读，编辑员可写用例但系统写接口返回 403，管理员可管理系统；默认管理员保持启用且不可降权或删除 |
| 个人信息 | 个人信息页面与 `/api/auth/me` | 管理员和编辑员可维护自己的本地显示名称和邮箱，查看者及 LDAP 资料只读；角色、状态和身份不可自助修改 |
| API 调用统计 | 个人信息页面 | SQLite 聚合保存平台今日/累计、开放、认证、未认证及当前用户认证调用次数 |
| LDAP 登录 | LDAP 页面与 `ldapts` | 服务账户搜索用户 DN，再使用用户密码 Bind；支持 Group Search Base/Filter 并同步简洁组名 |
...
1. 预检在本机逐表解析，校验 `data` 或连续用户旅程 Step、CaseID、srNum、重复值和对应 srNum 模板，并统计新增、将覆盖和内容未变化的行。未通过校验的表格会被排除并记录原因，只要仍有有效表格就可以继续。
2. 确认冲突策略后进入后台队列。任务进度与结果保存在 SQLite 中，浏览器关闭不会中断；进程异常退出时，未完成任务会在下次访问任务接口后重新排队。

“覆盖已有用例”会更新冲突记录并保存永久版本历史；“跳过已有用例”只写入新增记录；“遇到冲突终止”会在执行前再次检查，发现冲突时不写入用例。每个有效表格在真正写入前还会再次校验；某个表格此时失效时只排除该表，其余有效表格继续处理。预检文件最多保留 24 小时，完成、取消或失败后会清理位于持久化数据目录中的临时文件，但表格名称、状态和失败原因会永久保留在 SQLite 导入来源记录中。
